            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <?= $this->Html->link(
                        ...
                        )).__('Terminverwaltung'), '/termin', array('class' => 'nav-link active', 'escape' => false)) ?>
                </li>
                <li class="nav-item">
                    <?= $this->Html->link(
                        $this->Html->tag('i', '', array(
...
<div class="main-content">
    ...
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
        <div class="container-fluid">
            <!-- Brand -->
            <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="./index.html"><?php echo __('Termine') ?></a>
            <!-- User -->
                ...
                <div class="card shadow">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0"><?php echo __('Alle Meetings')?></h3>
                            </div>
                            <div class="col-4 text-right">
                                <button class="btn btn-sm btn-primary" type="button" onclick="location.href='./termin/add'">
                    ...
                    <div class="table-responsive">
                        <table class="table align-items-center text-center table-flush">
                            ...
                            <tr>
                                ...
                                <th scope="col"><?php echo __('Datum')?></th>
                                <th scope="col"><?php echo __('Art')?></th>
                                <th scope="col"><?php echo __('Bezeichnung')?></th>
                                <th scope="col"><?php echo __('Projekt')?></th>
                            ...
                            <?php foreach ($termin as $item): ?>
                                <tr>
                                    <td>
                                        <div class="avatar-group">
                                            ...
                                            foreach ($kundenliste as $k) {
                                                if ($item->projekt->kunde_id == $k['kunde_id']) {
                                                    print('<a href="kunde/edit/'.$item->projekt->kunde_id.'" class="avatar avatar-sm" data-toggle="tooltip" data-original-title="'.$k['name'].'">');
                                    ...
                                    <td><b><?=$item->datum?> <?php echo __('Uhr')?></b></td>
                                    <td><?=$item->art?></td>
                                    <td><?=$item->bezeichnung?></td>
                                    <td><?=$item->projekt->projektname?></td>
                                    <td>
                                        <div class="avatar-group">
                                            <?php
                                            foreach ($angestellterTermin as $item2) {
                                                foreach ($zustaendige as $zustaendig) {
                                                    if ($item2['angestellter_id'] == $zustaendig['angestellter_id'] && $item->termin_id == $item2['termin_id']) {
                                                        print('<a href="angestellter/edit/'.$item2['angestellter_id'].'" class="avatar avatar-sm" data-toggle="tooltip" data-original-title="'.$zustaendig['vorname'] .' ' . $zustaendig['nachname'].'">');
                                                        print('<img alt="Image placeholder" src="/~bolte/cakephp/extern/webroot/img/profilbilder/'.$zustaendig['username'].'.jpg" class="rounded-circle">');
                                                        print('</a>');
                                                    }
                                                }
                                            }
                                    ...
                                    <td class="actions">
                                        <?= $this->Html->link($this->Html->tag('i', '', array(
                                            ...
                                            ).__(''), ['action' => 'edit', $item->termin_id], array('escape' => false)) ?>
                                        <?= $this->Form->postLink($this->Html->tag('i', '', array(
                                                'class' => 'fas fa-trash'
                                                )
                                            ).__(''), ['action' => 'delete', $item->termin_id], array('escape' => false)) ?>
                                        <!-- <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $item->termin_id], ['confirm' => __('Are you sure you want to delete # {0}?', $item->termin_id)]) ?> -->
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
